...
// TestThatSinglefileUploadWorks checks whether Signle.HandleUpload doesn't let users
...
func (t *SingleTest) TestThatSingleAvatarUploadWorks() {
	...
	// Ensure low resolution file cannot be uploaded.
	t.PostFile("/single/HandleUpload", url.Values{}, url.Values{
		"file": {
	...
	// Check whether correct file is uploaded.
	t.PostFile("/single/HandleUpload", url.Values{}, url.Values{
		"file": {
